...
class dibs_pw_api extends dibs_pw_helpers {
    ...
    private function api_dibs_invoiceFields(&$aData, $mOrderInfo) {
		...
		$calculatedPrice += $iTmpPrice * $oItem->qty;
                unset($iTmpPrice);
            }
	
	     $amount = self::api_dibs_round($this->cms_dibs_applyCurrency($mOrderInfo['total'], $mOrderInfo['currency_code']));
            
             if($amount != $calculatedPrice) {
                 $delta = $amount - $calculatedPrice;
                 $aData['oiRow' . $i++] = 
                        1 . ';' . 
                        'pcs;' . 
                        'Rounding' . ';' .
